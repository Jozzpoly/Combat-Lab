...
export function startDrive(actor){
  if(actor.action.mode!=="idle") return false;
...
export function stepDrive(actor,dt,{
  homing=false,
  aimX,
  aimY,
  carryScale=1
  ...
  if(action.mode==="prepare"){
    ...
    if(action.time<=0){
      ...
      const delta=
        DRIVE_SPEC.carryImpulse*
        Math.max(0,carryScale)/
        Math.max(1,actor.spec.mass);
...
export function applyDriveContact(attacker,target,candidate,tuning){
  if(!candidate) return null;
  attacker.action.contactResolved=true;

  const scale=Math.max(0,tuning.driveMaterialScale);
  const targetDirX=-candidate.nx;
  const targetDirY=-candidate.ny;

  const invTarget=directionalInvMass(
    target,
    targetDirX,
    targetDirY,
    {
  ...
  const invAttacker=directionalInvMass(
    attacker,
    candidate.nx,
    candidate.ny,
    {
  ...
  const targetSupport=supportAlignment(
    target,
    targetDirX,
    targetDirY,
    {
