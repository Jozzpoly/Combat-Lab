...
test("E0b broad matched ablations isolate directionality support sacrifice and captured commitment",()=>{
  ...
  const summary={
    ...
    blindDirectional:compact(result.blindDirectional),
    blindOmni:compact(result.blindOmni),
    blindNoSupport:compact(result.blindNoSupport),
    blindDirectionVsNoSupport:result.blindDirectional.map((value,i)=>({
      offset:offsets[i],
      directionalResult:value.result,
      noSupportResult:result.blindNoSupport[i].result,
      directionalTime:value.time,
      noSupportTime:result.blindNoSupport[i].time,
      timeDelta:Number((value.time-result.blindNoSupport[i].time).toFixed(3)),
      directionalContacts:value.playerDriveContacts,
      noSupportContacts:result.blindNoSupport[i].playerDriveContacts,
      support:value.maxAdversarySupport
    })),
